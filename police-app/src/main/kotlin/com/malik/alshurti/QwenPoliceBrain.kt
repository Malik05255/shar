 ...
 *
 * Normal turns are generated by Qwen from the child's current sentence + recent
 * conversation. There is deliberately no table of canned behaviour replies.
 * A deterministic emergency guard remains before the model because safety should
 * never depend solely on a small local LLM.
 */
class QwenPoliceBrain(context: Context) : PoliceBrain {
    private val appContext = context.applicationContext
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
    private val started = AtomicBoolean(false)
    private val ready = CompletableDeferred<LlamaModel>()
    ...
    override suspend fun reply(userText: String): PoliceReply {
        ...
        return PoliceReply(generated, inferMood(cleanUser, generated))
    }
    ...
    private fun sanitize(raw: String): String {
        ...
        // A phone conversation sounds scripted when a tiny model rambles. Keep only
        // the first compact conversational beat, but do not replace it with canned text.
